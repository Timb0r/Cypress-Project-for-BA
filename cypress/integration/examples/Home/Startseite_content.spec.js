...
describe('Check homepage and content', () => {
    it('visits homepage', () => {
        ...
        cy.get('a[href*="#/übermich"]')
            .should('be.visible'),
        cy.get('a[href*="#/beratung"]')
            .should('be.visible'),
        cy.get('a[href*="#/"]')
            .should('be.visible'),
        cy.get('a[href*="#/service"]')
            .should('be.visible'),
        cy.get('a[href*="#/kontakt"]')
            .should('be.visible')
        ...
        cy.get('.contentRow').within(() => {
            ...
            contentBox1
        cy.contains('Die Entscheidung zum Kauf oder Bau einer Immobilie gehört zu den wichtigsten Entscheidungen im Leben. Dabei sollte Ihr Anspruch an die Finanzierung entsprechend hoch sein.')
            .should('be.visible')
        })
        cy.get('div.contentRow').within(() => {
            const contentBox2 = cy.get('div.contentCard').next()
            contentBox2
        cy.contains('Ihr Partner für unabhängige Immobilien­finanzierung')
            .should('be.visible')
        })
        cy.get('div.contentRow').within(() => {
        ...
        cy.url().should('contain', 'https://cy-borg.net/hortt-dev/#/%C3%BCbermich')
        cy.go('back')
        }) 
        ...
        cy.get('h2')
            .should('contain', 'Bankfachwirt') // find solution for checking the whole text with br between //
        })      
    })
