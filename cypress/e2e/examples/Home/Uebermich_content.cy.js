...
describe('Check aboutme page and content', () => {
    it('visits aboutme', () => {
        ...
        cy.get('div.contentRow').within(() => {
            ...
            contentHeadline
            cy.contains('Lernen Sie mich kennen').should('be.visible')
            cy.get('h1').should('contain', 'Lernen Sie mich kennen') // double check maybe just leave it out //
            cy.get('h2').should('contain', 'Manfred Hortt')
            cy.contains('Ihr Berater in Köln und Umgebung').should('be.visible')
